...
class models_helpers_Options
{
    ...
    public static function OrderStatuses($val = null)
    {
        $statuses = array(
            ...
            2 => models_helpers_Language::instance()->getOption('confirmed'),
            3 => models_helpers_Language::instance()->getOption('payed'),
            ...
            7 => models_helpers_Language::instance()->getOption('returned')
        );
    ...
    static public function getCatGroups()
    {
        $res = Database::instance()->select(models_BaseModel::TBL_CATEGORIES, 'DISTINCT(category) as category', 'WHERE status = 1 ' . 'ORDER BY name ASC', null, null, Database::ENCODE_HTML);

        if (!empty($res)) {
            foreach ($res as $key => $val)
                $options[$val['category']] = $val['category'];
            return $options;
        } else
            return array(0 => models_helpers_Language::instance()->getOption('not_found'));
    }

    static public function getCategories($category = '')
    {
        if ($category)
            $sql = 'AND category="' . $category . '"';

        $res = Database::instance()->select(models_BaseModel::TBL_CATEGORIES, 'id, name, parent_id, child, level', 'WHERE status = 1 ' . $sql . ' ORDER BY id ASC', null, null, Database::ENCODE_HTML);
        $res = self::sortAsTree($res, 0);
    ...
    static public function getCategoriesGtoups()
    {
        $res = Database::instance()->select(models_BaseModel::TBL_CATEGORIES, 'DISTINCT(category)', 'WHERE status = 1 ' . 'ORDER BY id ASC', null, null, Database::ENCODE_HTML);
    ...
    {
        $res = Database::instance()->select(models_BaseModel::TBL_TEMPLATES, 'name, id', 'WHERE status = 1 ORDER BY id ASC', null, null, Database::ENCODE_HTML);
        if (!empty($res)) {
            foreach ($res as $key => $val)
    ...
    {
        $res = Database::instance()->select(models_BaseModel::TBL_MODULES, 'DISTINCT(category)', 'WHERE status = 1 ORDER BY id ASC', null, null, Database::ENCODE_HTML);
        if (!empty($res)) {
            foreach ($res as $key => $val)
                $options[$val['category']] = $val['category'];
            return $options;
    ...
    {
        $res = Database::instance()->select(models_BaseModel::TBL_MODULES, 'id,name', 'WHERE status = 1 ORDER BY id ASC', null, null, Database::ENCODE_HTML);
        if (!empty($res)) {
            foreach ($res as $key => $val)
                $options[$val['id']] = $val['name'];
            return $options;
    ...
    {
        $positions = simplexml_load_file($_SERVER['DOCUMENT_ROOT'] . models_helpers_Url::getDomain() . "template/settings.xml");
        if (!empty($positions)) {
            foreach (explode(',', $positions->positions) as $key => $val)
                $options[$val] = $val;
            return $options;
        } else
            return array(0 => models_helpers_Language::instance()->getOption('not_found'));
    ...
    {
        // if no size added to product - showing all size in other case - only of product type
